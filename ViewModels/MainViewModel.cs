...
namespace AstralLite.ViewModels
{
    public class MainViewModel : ObservableObject
    {
        ...
        private bool _publicRoomModeEnabled;
        ...
        public MainViewModel()
        {
            ...
            _playerNameSaveTimer.Tick += (_, __) =>
            {
                _playerNameSaveTimer.Stop();
                UserSettingsService.SavePlayerName(PlayerName);
            };
        ...
        private void InitializeWfpMode()
        {
            var enabled = UserSettingsService.LoadWfpEnabled(true);
            _isWfpUpdating = true;
            SetProperty(ref _publicRoomModeEnabled, !enabled);
            _isWfpUpdating = false;
            ApplyWfpSetting(enabled, false);
        }

        private void ApplyWfpSetting(bool enabled, bool persistSetting)
        {
            try
            {
                _isWfpUpdating = true;
                if (persistSetting)
                {
                    UserSettingsService.SaveWfpEnabled(enabled);
                }
            ...
            {
                MessageBox.Show($"WFP update failed: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
                _isWfpUpdating = false;
        ...
        private static ProcessMonitorService? GetProcessMonitorService()
        {
            return (System.Windows.Application.Current as App)?.ProcessMonitorService;
        }
        ...
        public bool PublicRoomModeEnabled
        {
            get => _publicRoomModeEnabled;
            set
            {
                if (SetProperty(ref _publicRoomModeEnabled, value) && !_isWfpUpdating)
                {
                    ApplyWfpSetting(!value, true);
                }
            }
        }

        public bool RelayModeEnabled
        {
            get => _relayModeEnabled;
            set => SetProperty(ref _relayModeEnabled, value);
        }
        ...
        private string ApplyRelayMode(string config)
        {
            if (!RelayModeEnabled)
            {
                return config;
            ...
            return UpsertFlagInConfig(config, "disable_p2p", "true");
        }

        private static string UpsertFlagInConfig(string config, string key, string value)
        {
            ...
            for (var i = 0; i < lines.Count; i++)
            {
                ...
                if (string.Equals(trimmed, "[flags]", StringComparison.OrdinalIgnoreCase))
                {
                    flagsIndex = i;
                }
            }
            ...
            if (flagsIndex >= 0)
            {
                var endIndex = nextSectionIndex >= 0 ? nextSectionIndex : lines.Count;
                for (var i = flagsIndex + 1; i < endIndex; i++)
                {
                    var trimmed = lines[i].Trim();
        ...
        private void JoinRoom(RoomConfiguration? room)
        {
            ...
            try
            {
                ...
                var configWithRelay = ApplyRelayMode(configWithHostname);

                NetworkService.Instance.Connect(configWithRelay);
        ...
        private void TogglePlayerNameEdit()
        {
            if (IsConnected)
            {
                return;
            ...
            PlayerNameEnabled = !PlayerNameEnabled;
        }
